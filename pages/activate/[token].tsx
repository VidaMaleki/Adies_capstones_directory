...
export default function Activate({ token }: { token: string }) {
  ...
  const activateAccount = useCallback(async () => {
    try {
      const { data } = await axios.put(activate_account, { token });
      ...
      {error && (
        <div>
          ...
          <div className="mt-4">
            <Link
              href="/auth"
              className="mt-4 bg-blue-500 text-white hover:bg-blue-700 text-md uppercase font-bold px-8 py-2 rounded"
            >
              Sign in
            </Link>
          </div>
